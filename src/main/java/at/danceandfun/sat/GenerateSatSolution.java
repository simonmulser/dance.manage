...
public class GenerateSatSolution {
	...
	private void sibsInSamePerformance(List<Course> courses,
		...
		for(int k = idList.size()-1 ; k >= 0; k--) {
			...
			for(Participant sib : participants.get(idList.get(k)).getSiblings() ) {
				counter = 0;
				for(int h = idList.size()-1; h >= 0; h--) {
					System.out.println("Vergleiche " + sib.getFirstname() + " " + sib.getLastname() + " " + sib.getPid() + " mit " + participants.get(idList.get(h)).getFirstname() + " " + participants.get(idList.get(h)).getLastname() + " " + sib.getPid() );
					if(sib.getPid() == participants.get(idList.get(h)).getPid()) {
						counter++;
					}
				}
			}
			System.out.println("Geschwisteranzahl: " + participants.get(idList.get(k)).getSiblings().size());
		...
		for(int i = 0; i < idList.size(); i++) {
			...
			for(Participant sibl : participants.get(idList.get(i)).getSiblings()) {
				for(int j = 0; j < idList_copyX.size(); j++) {
					System.out.println("Hier vergleichen wir ID: " + sibl.getPid() + " mit " + participants.get(idList_copyX.get(j)).getPid());
					if(sibl.getPid() == participants.get(idList_copyX.get(j)).getPid()) {
						System.out.println("Sibl der nicht aufgenommen wird: " +participants.get(idList.get(i)).getLastname() + " " + participants.get(idList.get(i)).getPid());
			...
			if(!siblingsAlreadyIn) {
				...
				System.out.println("+1 Teilnehmer die darein fallen, zwar: " + participants.get(idList.get(i)).getPid() + " abgespeichert als: " + idList.get(i));
			}
		}		
		idList = idList_copyX;
		...
		for (int id : idList) {
			...
			for (CourseParticipant currentCP : currentParticipant
				...
				for (int i = 0; i < courses.size(); i++) {
					if (courses.get(i).equals(currentCP.getCourse())) {
						courseIDList.add(i);
						System.out.println("XXXXXXXXXXXX----> " + currentCP.getCourse());
					}
				}
			}
			...
			boolean courseIsIn;
			for(Participant sib : participants.get(idList.get(counterX)).getSiblings()) {
				for (CourseParticipant currentCP : sib.getCourseParticipants()) {
					courseIsIn = false;
					for (int i = 0; i < courses.size(); i++) {
						if (courses.get(i).equals(currentCP.getCourse())) {
							for(int j = 0; j < courseIDList.size();j++) {
								if(courseIDList.get(j).equals(currentCP.getCourse())) {
									courseIsIn = true;
								}
							}
							if(!courseIsIn) {	
								System.out.println(currentCP.getCourse().getName() + " einerVonSiblingKursen");
								courseIDList.add(i);
							}
						}
					}
