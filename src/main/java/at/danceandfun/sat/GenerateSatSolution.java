...
public class GenerateSatSolution {
	...
	private void multipleGroupsInSamePerformance(List<Course> courses,
		...
		System.out.println("Mein Name lautet: " + participants.get(idList.get(0)).getFirstname() + " " + participants.get(idList.get(0)).getLastname());
		...
		for (int id : idList) {
			...
			if (amount2.size() == amount3.size()
				...
				if (r.nextBoolean()) {
					...
					while (amount1.size() != 0) {
						checkAlreadySwapped(swappedCourses, amount1.get(0));
						Collections.swap(newOrderOfCourses, amount1.get(0), (2*numberOfSlots+helpSwapping(amount3)));
						amount3.add(amount1.get(0));
						swappedCourses.add(amount1.remove(0));
					}
				}
	...
	private int helpSwapping(List<Integer> listOfTakenCourses) {
		...
		System.out.println("1.helpSwapping Liste Elementanzahl: " +listOfTakenCourses.size() + " numberOfSlots: " + numberOfSlots);
		...
		for (int j = 0; j < listOfTakenCourses.size(); j++) {
			...
			System.out.println("3.list.size() " + list.size() + " numberOfSlots%listOfTakenCourses " + numberOfSlots + "  " + listOfTakenCourses.get(j));
	...
	private void checkAlreadySwapped(List<Integer> alreadySwapped, int courseID) throws SatException {
