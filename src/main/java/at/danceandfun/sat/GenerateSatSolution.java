...
public class GenerateSatSolution {
	...
	private void addBasicRestrictions(List<Course> courses, int k, int t, int p) {
		...
		/* Each slot is just allowed to be assigned to one style
		 * v = Logical OR
		 * a = Logical AND
		 * - = Negation
		 * Vi = Slots
	...
	private void addNotTwoOfAKind(List<Course> courses, int k, int t, int p,
		...
		/* The next course isn't allowed to contain the same style (just relevant for style 'Ballett)
		 * v = Logical OR
		 * - = Negation
		 * Vi = Slot
		 * Vj = Next Slot
	...
	private void sibsInSamePerformance(List<Course> courses,
		...
		for (int id : idList) {
			...
			System.out.println("Groesse der swappedList: " + swappedCourses.size());
			System.out.println("Teilnehmer: " + participants.get(id).getLastname() + " ID: " + id);
	...
	private void swapCourses(List<Integer> amount1, List<Integer> amount2, List<Integer> amount3) throws SatException {
		Random r = new Random();
		...
		if (amount1.size() > amount2.size()
				&& amount1.size() > amount3.size()) {
			System.out.println("Du bist hier: amount1.size() > amount2.size() && amount1.size() > amount3.size()");
			while (amount2.size() != 0) {
				checkAlreadySwapped(swappedCourses, amount2.get(0));
				Collections.swap(newOrderOfCourses, amount2.get(0),
						helpSwapping(amount1));
		...
		if (amount2.size() > amount1.size()
			...
			System.out.println("Du bist hier: amount2.size() > amount1.size() && amount2.size() > amount3.size()");
			while (amount1.size() != 0) {
				checkAlreadySwapped(swappedCourses, amount1.get(0));
				Collections.swap(newOrderOfCourses, amount1.get(0),
			...
			for(int i=0; i < amount2.size(); i++){
				System.out.println("Diese Kurse sind drinnen: " + newOrderOfCourses.get(amount2.get(i)).getName());
			}
			return;
		}
		if (amount3.size() > amount2.size()
				&& amount3.size() > amount1.size()) {
			System.out.println("Du bist hier: amount3.size() > amount2.size() && amount3.size() > amount1.size()");
			while (amount1.size() != 0) {
				checkAlreadySwapped(swappedCourses, amount1.get(0));
				Collections.swap(newOrderOfCourses, amount1.get(0),
